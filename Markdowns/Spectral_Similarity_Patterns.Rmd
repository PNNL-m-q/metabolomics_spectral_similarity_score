...
SummaryStats <- do.call(bind_rows, lapply(ScoreMetadata$Score, function(x) {
  c(
    Score = x,
    Min = Sum[[x]] %>% as.numeric() %>% min() %>% round(4),
    Median = Sum[[x]] %>% as.numeric() %>% median() %>% round(4),
    Max = Sum[[x]] %>% as.numeric() %>% max() %>% round(4)
  )
}))
...
Since both approaches produce the same clusters, we will run one random forest 
to explain cluster patterns. 

```{r}
library(randomForest)

ScoreMetadata$SumCluster <- as.factor(ScoreMetadata$SumCluster)
ScoreMetadata$Family <- as.factor(ScoreMetadata$Family)
ScoreMetadata$TheoreticalBounds <- as.factor(ScoreMetadata$TheoreticalBounds)
ScoreMetadata$Type <- as.factor(ScoreMetadata$Type)

rf <- randomForest(SumCluster ~ Family + Type + TheoreticalBounds + Min + Median + Max, data = ScoreMetadata, mtry = 1000) 

rf
importance(rf) %>% data.frame() %>% arrange(-MeanDecreaseGini)
```
