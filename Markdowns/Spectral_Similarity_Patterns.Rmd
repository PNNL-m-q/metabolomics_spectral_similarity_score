...
ScoreMetadata <- ScoreMetadata %>% dplyr::select(-c(Sum_ScoreMin, Sum_ScoreMedian, Sum_ScoreMax))
...
**Sum Cluster:** The cluster number the score falls into for the sum data 

**MaxCluster:** The cluster number the score falls into for the max data 

**Min:** The score's minimum value

**Median:** The score's median value

**Max:** The score's max value
...
SumMetadata <- ScoreMetadata %>% 
  dplyr::select(SumCluster, Family, Type, TheoreticalBounds, Min, Median, Max, TStatisticSum) %>%
  dplyr::mutate(
