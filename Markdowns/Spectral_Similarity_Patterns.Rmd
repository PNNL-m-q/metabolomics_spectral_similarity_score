...
# Sub down to peak index
Sub <- Sum[Sum$`Sample name` == "UDN_BG_A_C_Biorec291064_M_012" & Sum$`Peak Index` == 13,] %>%
  mutate(
    Truth.Annotation = ifelse(Truth.Annotation == "True.Positive", "True Positive", Truth.Annotation),
    Truth.Annotation = ifelse(Truth.Annotation == "True.Negative", "True Negative", Truth.Annotation)
  )

Sub %>%
  dplyr::select(`Stein Scott Similarity`, `DFT Correlation`, `DWT Correlation`, `Cosine Correlation`, Truth.Annotation) %>%
  tidyr::pivot_longer(1:4) %>%
  dplyr::rename(`Score Name` = name, `Score Value` = value, `Truth Annotation` = Truth.Annotation) %>%
  ggplot(aes(x = `Score Name`, y = `Score Value`)) +
   geom_boxplot(outlier.shape = NA) +
   geom_jitter(height = 0, width = 0.1, aes(color = `Truth Annotation`), size = 2) +
   theme_bw() +
   ylim(c(0, 1)) +
...
TP_Rankings <- Sum %>%
  ...
  select(all_of(c(ScoreMetadata$Score, "Truth.Annotation"))) %>%
  mutate(
    `Kumar Johnson Divergence` = as.numeric(`Kumar Johnson Divergence`)
  ) %>%
  pivot_longer(3:68) %>%
  ungroup() %>%
  group_by(`Sample name`, `Peak Index`, name) %>%
  mutate(
    Rank = rank(value)
  ) %>%
  ungroup() %>%
...
TP_Rankings %>%
  rename(Score = name) %>%
  merge(ScoreMetadata %>% select(Score, SumCluster)) %>%
  mutate(`Cluster Number` = as.factor(SumCluster)) %>%
  arrange(`Cluster Number`) %>%
  mutate(Score = factor(Score, levels = unique(Score))) %>%
  ggplot(aes(x = Score, y = Rank, fill = `Cluster Number`)) +
    geom_boxplot() +
    theme_bw() +
    ylab("True Positive Rank") +
