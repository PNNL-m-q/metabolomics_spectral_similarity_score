...
Fig1 <- make_ms_plot(Query, "Query Spectra is L-Alanine", scale = T) +
  make_ms_plot(Glycolic, "Glycolic Acid, Spectral Similarity = 0.761") +
  make_ms_plot(Lactic, "Lactic Acid, Spectral Similarty = 0.588") +
  make_ms_plot(Alanine, "L-Alanine, Spectral Similarty = 0.165") +
...
```{r}
SumModel[[4]]$Variable <- c("T Statistic", "Overlap", "Median Value", "Min Value", 
                       "Theoretical Bounds", "Score Type", "Family", "Max Value")
SumModel[[4]]$Variable <- factor(SumModel[[4]]$Variable, levels = rev(SumModel[[4]]$Variable))
MaxModel[[4]]$Variable <- c("T Statistic", "Overlap", "Median Value", "Max Value", 
                       "Theoretical Bounds", "Min Value", "Family", "Score Type")
MaxModel[[4]]$Variable <- factor(MaxModel[[4]]$Variable, levels = rev(MaxModel[[4]]$Variable))

(ggplot(SumModel[[4]], aes(x = Importance, y = Variable)) + 
    geom_bar(stat = "identity") + theme(legend.position = "none") +
    theme_bw() + ggtitle("Sum")) + 
(ggplot(MaxModel[[4]], aes(x = Importance, y = Variable)) + 
    geom_bar(stat = "identity") + theme(legend.position = "none") +
    theme_bw() + ggtitle("Max")) 
```
